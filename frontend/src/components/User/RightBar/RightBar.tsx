import {TextField,Theme,Box, Avatar,Stack,Typography} from "@mui/material"
...
const RightBar = () =>{
    ...
    return (
      <Box className={classess.container} sx={{borderLeft:'1px groove'}}  >
       <Box px={8}>
       <TextField id="outlined-search" label="find friends..." type="find" fullWidth 
       autoComplete="off"
       value={searchValue}
      ...
      <Box p={5} ml={5}>
        {
         users.map((user)=>(
            ...
            <Stack direction={'row'} sx={{alignItems:'center',cursor:'pointer'}} spacing={1} mt={2} key={user._id} 
            onClick = {()=>handleNavigate(user._id)}
             >
            <Avatar sx={{bgcolor:getRandomColor()}}>{user.userName[0]}</Avatar>
            <Typography variant="h6" sx={{fontWeight:"bold",color:theme.palette.primary.main}}>{user.userName}</Typography>
            </Stack>
         ))
        }
      </Box>
